...
# ============================================================================
# This is the old update, kept for reference
# ============================================================================
...
class SearchOptimizer:
    def __init__(self, characters: Dict[str, List[str]], levels: Dict[str, List[int]]):
        ...
        self.l3_capable_traits = {trait for trait, lvls in levels.items() if max(lvls) >= 6}
        self.explored_count = 0

    def calculate_active_traits(self, trait_counts: Counter) -> Tuple[List[str], int, int, int]:
        """
        Calculate active traits (only highest level per trait)
        Returns: (trait_list, num_l1, num_l2, num_l3)
        """
        active_traits = {}
        ...
        num_l3 = sum(1 for level in active_traits.values() if level == 6)

        trait_list = sorted([f"{trait}_{level}" for trait, level in active_traits.items()])

        return trait_list, num_l1, num_l2, num_l3
    ...
    def search_0_l2(self, num_chars: int, dummy: Optional[List[str]] = None) -> List[Dict]:
        ...
        def dfs(team: List[str], remaining: List[str], trait_counts: Counter):
            ...
            if len(team) == num_chars:
                active_traits, num_l1, num_l2, num_l3 = self.calculate_active_traits(trait_counts)
                if num_l2 == 0 and num_l3 == 0:  # Pure L1
    ...
    def search_n_l2(self, num_chars: int, target_l2_count: int, dummy: Optional[List[str]] = None) -> List[Dict]:
        """
        Search for teams with exactly target_l2_count L2 traits (level 4, NOT level 6)
        """
        ...
        def search_with_l2_targets(l2_targets: List[Tuple[str, int]]):
            ...
            def dfs(team: List[str], remaining: List[str], trait_counts: Counter):
                ...
                # Base case
                if len(team) == num_chars:
                    # Check if we achieved all L2 targets (exactly 4, not 6)
                    achieved = True
                    for trait, req in target_traits.items():
                        if trait_counts[trait] < 4:
                            achieved = False
                            break
                        # Reject if this becomes L3
                        if trait_counts[trait] >= 6:
                            achieved = False
                            break

                    if achieved:
                        active_traits, num_l1, num_l2, num_l3 = self.calculate_active_traits(trait_counts)
                        if num_l2 == target_l2_count and num_l3 == 0:
                            total = len(active_traits)
                            if total > max_traits:
                                max_traits = total
                                best_teams = [
                ...
                # Check if we can still achieve L2 targets without exceeding to L3
                for trait, req in target_traits.items():
                    if trait_counts[trait] + slots_left < req:
                        return  # Can't reach target
                    if trait_counts[trait] >= 6:
                        return  # Already at L3, reject
    ...
    def search_n_l3(self, num_chars: int, target_l3_count: int, dummy: Optional[List[str]] = None) -> List[Dict]:
        """
        Search for teams with exactly target_l3_count L3 traits (level 6)
        Only possible with dummy that has noble and/or undead
        """
        print(f"\nSearching for {target_l3_count} L3 teams...")
        self.explored_count = 0
        best_teams = []
        max_traits = 0

        # Get traits that can reach L3 (level 6) - only noble and undead
        l3_candidates = [(t, 6) for t in self.l3_capable_traits]

        if not l3_candidates:
            print(f"  No L3-capable traits available")
            return []
        ...
        def search_with_l3_targets(l3_targets: List[Tuple[str, int]]):
            """Search for teams achieving specific L3 targets"""
            nonlocal max_traits, best_teams

            target_traits = {t[0]: t[1] for t in l3_targets}
            ...
            def dfs(team: List[str], remaining: List[str], trait_counts: Counter):
                ...
                # Base case
                if len(team) == num_chars:
                    # Check if we achieved all L3 targets
                    achieved = sum(1 for trait, req in target_traits.items() if trait_counts[trait] >= req)

                    if achieved == len(target_traits):
                        active_traits, num_l1, num_l2, num_l3 = self.calculate_active_traits(trait_counts)
                        if num_l3 == target_l3_count:
                            total = len(active_traits)
                            if total > max_traits:
                                max_traits = total
                                best_teams = [
                ...
                # Check if we can still achieve L3 targets
                for trait, req in target_traits.items():
                    if trait_counts[trait] + slots_left < req:
                        return  # Can't reach target
        ...
        # Try different combinations of L3 target traits
        from itertools import combinations

        for l3_combo in combinations(l3_candidates, target_l3_count):
            search_with_l3_targets(list(l3_combo))
    ...
    def calculate_max_l2_traits(self, num_chars: int, dummy: Optional[List[str]] = None) -> int:
        ...
        if dummy:
            total_chars += 1  # Dummy counts as a character

        # Calculate max intersections
        if dummy:
            max_intersections = 3
        else:
            max_intersections = 2

        # For N L2 traits: need 4*N - max_intersections <= total_chars
        # Solve for N: N <= (total_chars + max_intersections) / 4
        max_l2 = (total_chars + max_intersections) // 4

        # Also limited by number of L2-capable traits
    ...
    def find_all_optimal_teams(self, num_chars: int, dummy: Optional[List[str]] = None) -> Dict[str, List[Dict]]:
        """Find all optimal teams for each L1/L2/L3 category"""
        result = {}
        ...
        # Check if L3 is possible (requires dummy with noble or undead)
        l3_possible = False
        if dummy:
            for trait in dummy:
                if trait in self.l3_capable_traits:
                    l3_possible = True
                    break

        if l3_possible:
            # Calculate max L3 - need 6 of a trait, dummy provides 1
            # So need 5 more characters with that trait
            max_l3 = min(len(self.l3_capable_traits), (num_chars + 1) // 6)
            print(f"Maximum possible L3 traits: {max_l3}")

        # Search for 0 L2, 0 L3 (pure L1)
        teams_0 = self.search_0_l2(num_chars, dummy)
        if teams_0:
            result["0_l2_0_l3"] = teams_0

        # Search for L2 only (no L3)
        for l2_count in range(1, max_l2 + 1):
            teams = self.search_n_l2(num_chars, l2_count, dummy)
            if teams:
                result[f"{l2_count}_l2_0_l3"] = teams

        # Search for L3 (can be combined with L2)
        if l3_possible:
            for l3_count in range(1, max_l3 + 1):
                # L3 only (no L2)
                teams = self.search_n_l3(num_chars, l3_count, dummy)
                if teams:
                    result[f"0_l2_{l3_count}_l3"] = teams

                # L3 + L2 combinations (if there's room)
                # For example: 1 L3 + 1 L2 if we have enough characters
                # Skip for now - this gets complex quickly
...
def main():
    ...
    for category in sorted(optimal_teams.keys(), key=lambda x: (int(x.split("_")[0]), int(x.split("_")[2]))):
        teams = optimal_teams[category]
        if teams:
            num_l1 = teams[0]["num_l1"]
            num_l2 = teams[0]["num_l2"]
            num_l3 = teams[0]["num_l3"]
    ...
    print("=" * 80)
    base_filename = f"{NUM_CHARACTERS}chars{'_dummy' if DUMMY else ''}"
